...
public interface LavadoRepository extends JpaRepository<Lavado, Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO lavado (id, precio, cantidadalavar) VALUES (:id, :precio, :cantidadalavar)", nativeQuery = true)
    void insertarLavado(@Param("id") Integer id, @Param("precio") Float precio,
            @Param("cantidadalavar") Integer cantidadalavar);

    @Modifying
    @Transactional
    @Query(value = "UPDATE lavado SET precio = :precio, cantidadalavar = :cantidadalavar WHERE id = :id", nativeQuery = true)
    void actualizarLavado(@Param("id") Integer id, @Param("precio") Float precio,
            @Param("cantidadalavar") Integer cantidadalavar);
